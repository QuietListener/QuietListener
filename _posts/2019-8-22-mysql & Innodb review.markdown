...
INNODb的意向锁实现是**表级别的**
IX IS S X 互斥性    这里的S X是指**表级锁**
**意向锁不会与行级的S/X锁互斥**
![部署](https://raw.githubusercontent.com/QuietListener/quietlistener.github.io/master/images/20200624-ixislock.jpg)


#### 1. 意向锁要解决的问题(为什么要有意向锁)
表T
1. 事务1 已经对行r加了X锁，并没有释放。
2. 事务2 要对表T加S锁(LOCK TABLES T READ;)

事务2在加S锁之前要保证表上没有任何行有X锁，如果没有意向锁的话，事务2就需要查看所有行的锁情况，看是不是有X锁，如果没有才能加，**这样效率非常低**
试想有了意向锁 在**事务1**对r加了X锁，那必然对表T加了IX锁， **事务2**发现表上有IX锁就知道表中一定至少有某些行有X锁，所以**事务2**必须等待。这样就不用查看所有行的加锁情况了。

注意:
1. **意向锁不会与行级的S/X锁互斥**
2. **意向锁不会与行级的S/X锁互斥** 多说两遍很重要
3. **IX IS 都是兼容的** 


### 3. 一致性非锁定读
Innodb使用多版本控制来读取当前执行时间数据库中的数据，如果读取的行正在进行delete或者update操作，读取不用等待锁释放，而是去读取行的一个快照数据。
**非锁定读**，由于是读快照，不需要等待行上的X锁的释放，大大增加了并发性。

快照就是当前行数据之前的历史版本, 没行可能有多个版本，由此带来的并发控制叫 多版本并发控制技术(**MVVC**)

在事务隔离级别为read committed 和 repeatable read下非锁定一致性读的定义是不同的，read committed是读取最新的一次快照，repeatable read是读取事务开始时的行数据版本。



### 4. 一致性锁定读
innodb对select语句支持2总一致性的锁定读:
1. select  ... for update 对读取记录加一个X锁
2. select .... lock in share mode 对读取记录加S锁
...
## 锁的问题
锁的引入保证了隔离性，使得数据库可以并发工作。也会带来问题.
### 1. 脏读问题
读取了还没有提交的数据，

### 2. 不可重复度问题:
在一个事务内多次读取同一个数据集合，在这个事务还没有结束的时候，另一个事务也访问了同一个数据集合，并做了dml操作。在第一个事务两次读取数据之间，由于第二个事务的修改，第一个事务两次读取的数据不一样，就叫做**不可重复度**。 **不可重复读是读取的一句提交的数据。**，在Mysql中将**不可重复读**定义为**幻读**问题。使用next-key locking机制来避免。

### 3. 阻塞
 由于锁有兼容性关系，有时候一个事务需要等待另一个事务释放锁资源，就是产生**阻塞**。
 可以由下面的参数控制所等待时间。
...
1 row in set (0.13 sec)
 ...
 当发生锁等待timeout时候是会馆还是不回滚可以由innodb_rollback_on_timeout控制
 默认情况下是OFF，是不回滚，而是抛出异常，由应用程序来决定commit还是会馆。
 ```sql
...
###  4. 死锁
死锁是两个或者以上事务在争夺资源而相互等待的现象。
解决死锁的办法有**超时**，和**等待图(wait-for graph)**

1. 超时的方法:
超时是一种**被动**的死锁检查机制。 等待操作阈值后，选址一个事务进行回滚，但是超时还是比较暴力

2. 等待图:wait-for graph
等待图是一种**主动**的死锁检查机制。如果等待图中有回路就表示有死锁存在，这个时候一般选择undo量最少的事务进行回滚。等待图Innodb采用深度优先算法来实现。  


tip:
**innodb多大部分错误不会回滚，但是死锁会回滚其中一个事务**



# 事务
### 1. 什么是事务
事务是一组sql语句组成，是访问并更新数据库中各种数据项的一个执行单元，在事务中的操作，要么都做修改，要么都不做，这也是事务的目的


理论上事务有很严格的定义: 他必须同时满足:ACID四个特性。但是很多数据库厂商都没有严格满足。   
#### 1. A Acomicity 原子性
 事务中的sql语句要么全部执行成功，要么一条都不执行。如果有执行失败，必须回滚到事务执行前的状态。   
#### 2. C consistency 一致性。
数据库必须重一种一致的状态转换到下一个一致的状态，在事务开始之前和事务结束之后**数据库的完整性约束**没有被破坏。例如表中学号列是唯一的，如果事务对学号进行了修改，如果在事务提交或者回滚后学号不唯一了，就破坏了一致性要求。   
#### 3. I Isolation 隔离性
隔离性要求美国读写事务的对象对其他事务的炒作对象能相互分离，即事务提交前对其他事务都不可见。**通常由锁来实现**。  
#### 4. Durability 持久性
事务一旦提交，结果就是永久性的，就算宕机也能恢复。Innodb通过 binlog，redo log，double write等级机制来保证持久性。
...
#### 1. redo保证D 持久性
重做日志有两部分组成: 1. 重做日志缓冲(内存里易失)，2. 重做日志文件(磁盘里持久的)。
...
0 表示提交时不写重做日志，有master thread定时来完成(每秒一次)
2 表示写重做日志，但是不强制调用fsync刷盘。

```sql
mysql> show variables like "%_at_trx%";
+--------------------------------+-------+
| Variable_name                  | Value |
+--------------------------------+-------+
| innodb_flush_log_at_trx_commit | 1     |
