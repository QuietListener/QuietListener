...
# 5. 正规方程(normal equation)，直接求出 $J(θ)$ 最小值时候的 θ值
## 1. 基本思想
...
**Andrew Ng的建议:** 一般当特性数量下雨10000 直接使用正规方程，如果n太大，还是使用梯度下降。

# 6. 正规方程时候遇到 矩阵 不可逆的情况
$ θ = (X^TX)^{-1} $ 如果$ (X^TX)^{-1} $不可逆。
## 1. 原因:
### 1. 有冗余的feature  
