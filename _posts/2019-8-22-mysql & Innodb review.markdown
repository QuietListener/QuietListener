...
2. 写锁是排他的，一个写锁会柱塞其他读锁和写锁，这样保证同一个时间只有一个用户能写入。
...
隔离界别规定了**每一种级别中“一个事务所做的修改，哪些是事务内核事务外可见的，哪些是不可见的。**
#### 1. READ UNCOMMITTED;
...
##### 2. insert操作 
插入的新行的**创建版本号**设置为 当前 统版本号 
##### 3. delete操作  
为删除行的 **删除本号**设置为 当前 统版本号
##### 4. update操作   
插入一个新行，将**创建版本号**设置为 **当前系统版本号**，设置原来行的**删除版本号**为 **当前系统版本号**
