...
### 2. 非线性决策边界
<img src="https://raw.githubusercontent.com/QuietListener/quietlistener.github.io/master/images/20200318-logistics-regression3.jpg" width="400"> 
...
**在 logistic regresion 中  0<z<1  (  $$ z = h_θ(x) $$  ) ，所以只取[0,1]这一段** 
如下面的图所示：  
...
例如**1表示是恶性肿瘤， $$ h_θ(x) $$  越大，表明预测越准，cost funtion应该越小，因为预测越正确，代价要越小**; $$ z=h_θ(x) $$  越小，越不满足预期，cost function 应该越大。  
 
 所以  $$ y = 1 的时候 cost(h_θ(x), y )   $$  应该是 关于   $$ h_θ(x) $$ 的**递减函数**。    
 **$$  cost(h_θ(x), y )  =  -log(h_θ(x)) 是满足条件的 $$**


## 2. 当 y=0 的时候 表示分为良性肿瘤  

 $$ z=h_θ(x) $$  越大，越不满足预期，cost function 应该越大。
 $$ z=h_θ(x) $$  越小，越满足预期，cost function 应该越小。

所以y = 0 的时候 $$cost(h_θ(x), y )$$  应该是 关于   $$ h_θ(x)$$  的**递增函数**。 
