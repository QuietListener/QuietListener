...
## 4. 具体
  ...
  ### 1. **原来的问题是:**
  1. $$J(θ) = \frac1{2m} \sum_{i=1}^{m} [  h_θ(x^{(i)} - y^{(i) }) ^2 ] $$

   2. 最小化 J(θ) 获取 θ值

   ### 1. **现在的问题是:**
  1. $$J_1(θ) = \frac1{2m} \sum_{i=1}^{m} [  h_θ(x^{(i)} - y^{(i) }) ^2   + λ \sum_{i=1}^m θ_i^2] $$
  ...
  **λ叫做正则化参数。**  λ其实就是控制 $$ h_θ(x^{(i)} - y^{(i) }) ^2 $$ 和  $$  \sum_{i=1}^m θ_i^2$$ 之间的取舍，在二者之间做一个trade off。
  ...
  如果我们将 λ 设置得非常大，比如10000000，最后训练出来的所有参数 $$θ_1,θ_2,θ_3,...,θ_n $$ 都接近0，那曲线就变成了 $$h_θ(x) = θ_0$$ **成了一条直线** ,又变成了**欠拟合(underfitting)**
